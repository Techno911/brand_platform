...
### 2026-04-20 — Card-slip под sticky-bar: «Ну, ты видишь, у тебя уехала» (v3.6.0)

**Что произошло.** После v3.5.9 (inline StageCompleteCard + PERSISTENT_BLOCKS) Артём открыл Стадию 2 КДМ под своим акком, проскроллил до активного challenge-таба и прислал скриншот: зелёная карточка «Все четыре блока утверждены» **частично уехала под StickySubmitBar**. Визуально — бежевая полоса сверху (наполовину скрытая header'ом страницы), зелёная рамка карточки обрезана снизу sticky-бар'ом с «Легенда Ценности Миссия 3/3 На одобрение собственника». Артём: «Ну, внимательнее, пожалуйста. Ну, ты видишь, у тебя уехала. Как так-то?» Якорь дошёл до пользователя → пользователь увидел якорь → якорь визуально сломан sticky-bar'ом → пользователь считает это очередной регрессией верстки, а не fix'ом.

**Корневая причина.** Я разместил StageCompleteCard внутри canvas-колонки grid'а (`grid-cols-[minmax(0,2fr)_minmax(0,3fr)_320px]`), сразу под FeedbackForm / EmptyCanvasPlaceholder. Плюсы размещения: «ровно где глаза после accept». Минусы которые не увидел: (a) canvas-колонка имеет неопределённую высоту, зависящую от контента (на challenge-табе без результата EmptyCanvasPlaceholder растягивает колонку на ~200px, на других табах — больше); (b) у grid'а общий `pb-24` (96px) для clearance от sticky-bar'а (high ~65px); (c) StageCompleteCard имеет высоту ~200px — если она оседает в нижней трети колонки и пользователь проскроллил так, что карточка оказалась в зоне `viewport.h - 96px..viewport.h`, sticky-bar перекрывает. Короче: pb-24 был рассчитан на footer-пустоту грида, а не на визуально-значимый контент внутри колонки.

Плюс методологический косяк: per-stage CTA — per-stage объект, а я засунул его в per-tab колонку. Ширина ~412px (w=cardRect.w на скриншоте Preview) визуально конкурировала с правым sidebar'ом «Подсказки по текущему шагу» и не давала ощущения «это главный следующий шаг на стадии». Узкий кард → воспринимается как «ещё один блок», широкий кард → воспринимается как «секция стадии».

**Что починил.** `frontend/src/pages/wizard/Stage2Page.tsx`: StageCompleteCard вынесен ИЗ canvas-колонки в отдельный full-width слот между `</Tabs>` и открывающим `<div className="grid ...">`. Рендер:
```tsx
</Tabs>
</div>
...
<div className="grid grid-cols-1 xl:grid-cols-[...] gap-6 mt-6 pb-24">
```
Теперь кард имеет ширину = ширина контента стадии (1280px на viewport 1440px), живёт ВНЕ грида, `pb-24` остаётся на гриде и не должен защищать кард от sticky-bar'а.

**Верификация Preview MCP (1440x900, Белая Линия Stage 2, challenge активен, 3/3 персистентных блока accepted):**
- `cardRect`: y=594, w=1280, h=184, bottom=778.
- `gridRect`: y=802 → карточка заканчивается за 24px ДО grid'а (cardBeforeGrid=true).
- `stickyRect`: top=1381 → карточка заканчивается за 603px ДО sticky-bar'а в любом viewport'е (cardAboveSticky=true, overlap=false).
- `cardIsFullWidth`: true (1280 == gridWidth).
- Скрин подтвердил: зелёная рамка сразу под Tabs, primary Button «На одобрение собственника» с Send-icon, ничего не перекрывает снизу.

**Правило.**
(a) **Per-stage / per-project CTA рендерится в изолированном полноширинном слоте, НЕ внутри grid-колонки.** Колонки грида имеют неопределённую высоту и общий `padding-bottom` на контейнере — любой визуально-значимый элемент внутри колонки рискует попасть в «зону `pb`» и быть перекрытым sticky-footer'ом. Правило семантическое: if CTA принадлежит стадии/проекту (не активной вкладке / не конкретному блоку) → он рендерится на уровне стадии/проекта, т.е. в полноширинной строке между header'ом страницы и grid'ом с колонками.
(b) **Визуальная ширина CTA кодирует семантический уровень.** Узкая карточка (треть-ширины, в колонке) = per-block действие. Средняя (полуколонка или inline в FeedbackForm) = per-block с опциями. **Full-width** = per-stage / per-project — это «главный следующий шаг для всей стадии». Инвариант: если уровень CTA = per-stage, ширина = полная ширина контента страницы, никак не меньше.
(c) **После любой правки размещения (moved / reorganized / новый компонент в layout) — обязательна verify через Preview MCP `preview_eval` + `preview_screenshot`, ДВА метода.** `preview_eval` для boundingBox / overlap / full-width-check (математика: `cardWiderThanGrid`, `cardAboveSticky`, `cardBeforeGrid`). `preview_screenshot` для sanity глазами (видно ли текст читаемо, не накладывается ли визуально). Один без другого — слепое пятно: eval не знает про z-index / opacity / скрытый элемент рядом; screenshot не знает про точные пиксели / скролл / hover-стейты. Оба ВМЕСТЕ = полная картина.
(d) **`pb-N` на grid-контейнере защищает только от sticky-bar, не от sibling-контента в грид-колонках.** Если элемент `A` внутри колонки 1 колонки grid'а с `pb-24`, а sticky-bar перекрывает последние 65px viewport'а — элемент `A` может оказаться в 31px-clearance-зоне и быть перекрытым при определённом scroll position'е. Решение НЕ «увеличить `pb` до `pb-40`» (это bloating), а **переместить важный контент из колонок в полноширинные secondary-контейнеры выше grid'а**. Правило расширяется: любая кнопка/CTA, которая живёт в колонке grid'а и выглядит финальной для стадии, проверяется на overlap с sticky при `scrollY = scrollHeight - viewport.h` (максимальный скролл).
